...
@pytest.mark.parametrize('exp_price, units, trade_type, order_id',
    [(Decimal('10020'), Decimal('0.21'), 'entry', 1332),
     (Decimal('9972'), Decimal('-0.33'), 'exit', 32)]
)
def test_init_(exp_price: Decimal, units: Decimal, trade_type: str,
        order_id: int) -> None:
    trade = Trade(exp_price, units, trade_type, order_id)
...
def test_fill_trade_long(exp_price: Decimal, units: Decimal, trade_type: str,
...
def test_fill_trade_short(exp_price: Decimal, units: Decimal, trade_type: str,
        order_id: int, exec_price: Decimal, delta_balance: Decimal,
        delta_upl: Decimal) -> None:
    trade = Trade(exp_price, units, trade_type, order_id)
    first, second = trade.fill_trade(exec_price)
